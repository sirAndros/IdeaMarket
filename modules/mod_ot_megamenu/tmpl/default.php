...
foreach ($items as $i => &$item) {
    ...
    if (isset($item->content) AND $item->content) {
		echo '<li class="ot-item'.' level' . $item->level .' '. $item->submenuList .' '.$item->classes .'">' . $item->content;
		$item->ftitle = '';
    } 
    ...
    if ($item->ftitle != "") {
		...
		echo '<li class="ot-item'.' level' . $item->level .' '. $item->submenuList .' '.$item->itemClassSuffix .' '.$item->classes .'">';
